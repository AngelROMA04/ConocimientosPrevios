...
void editCD(int code, CD CDs[50]) {
    int i = buscarCD(code, CDs);
    if (i >= 50) {
        cout << "CD no encontrado" << endl;system("PAUSE");
    }
    else {
        aggCD(i, code, CDs);
    }
...
int main()
{
    ...
    do {
        ...
        case '5':
            ...
            num = buscarCD(codeCHK, CDs);
            cout << "DATOS DEL CD: ";mostrarCD(num, CDs);system("PAUSE");system("CLS");
            if (CDs[num].existencias < 1) { cout << "No hay CDs disponibles que coincidan";
            } else { sellCD(codeCHK, CDs); }
            break;
        case '6':
            exit(0);
            break;
